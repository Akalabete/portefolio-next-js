  ...
  React.useEffect(() => {
    ...
    const updateDimensions = () => {
      if (typeof window !== "undefined") {
        setCenterHeight(window.innerHeight > 640 ? (window.innerHeight) / 2 - 160 - 20 : (window.innerHeight) / 2 - 64 - 20);
        setCenterWidth(window.innerWidth > 1280 ? (window.innerWidth) / 2 - 384 - 18 :
          window.innerWidth > 1024 ? (window.innerWidth) / 2 - 192 - 18 :
          window.innerWidth > 768 ? (window.innerWidth) / 2 - 144 - 18 :
          window.innerWidth > 640 ? (window.innerWidth) / 2 - 96 - 18 :
          (window.innerWidth) / 2 - 16 - 18);
      }
    };

    updateDimensions(); // Appel initial
    window.addEventListener('resize', updateDimensions); // Écouteur d'événements pour les redimensionnements

    return () => {
      window.removeEventListener('resize', updateDimensions); // Nettoyage de l'écouteur
    };
  }, []);
  return (
    <motion.div
      ...
      <div className="relative w-full  flex flex-col space-y-4">
        ...
        >
          ...
          <span className="text-gray-500 text-md">
            <span className="font-bold">www.lacouralexandre.tech </span>
            unexpectedly{" "}
            {ShowText ? (
              <motion.span
